...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    private function __construct(){
        ...
        $this->stmt_incremento= $this->dbh->prepare("update Usuarios set saldo=saldo+(saldo*0.1) where nombre=?");
        $this->stmt_bloqueo   = $this->dbh->prepare("update Usuarios set bloqueo=1 where nombre=?");
    }
    ...
    public function addUsuario($user):bool{
        ...
        $this->stmt_creauser->execute( [$user->login, $user->nombre, $user->password, $user->bloqueo, $user->saldo, $user->comentario]);
    ...
    //FUNCIONES
    function incrementarSaldo ($login) {
        $this->stmt_incremento->bindValue(1, $login);
        $this->stmt_incremento->execute();
        $resu = ($this->stmt_incremento->rowCount () == 1);
        return $resu;
    }

    function bloqueo ($login) {
        $this->stmt_bloqueo->bindValue(1, $login);
        $this->stmt_bloqueo->execute();
        $resu = ($this->stmt_bloqueo->rowCount () == 1);
        return $resu;
    }
}
